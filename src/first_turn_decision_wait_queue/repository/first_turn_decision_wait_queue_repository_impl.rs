...
impl FirstTurnDecisionWaitQueueRepository for FirstTurnDecisionWaitQueueRepositoryImpl {
    async fn enqueue_player_tuple_for_wait(&self, player_tuple:(i32,String)) -> Result<bool, Box<dyn Error>> {
        println!("FirstTurnDecisionWaitQueueRepositoryImpl: enqueue_player_tuple_for_wait()");
        let first_turn_decision_waiting_queue_guard = self.first_turn_decision_wait_queue.lock().await;
        first_turn_decision_waiting_queue_guard.enqueue_player_tuple(player_tuple).await;
    ...
    // async fn dequeue_two_players_id_from_wait_queue(&self, count: usize) -> Vec<(i32,String)> {
    ...
    // }
    // async fn dequeue_two_players_choice_from_wait_queue(&self, count: usize) -> Vec<(i32,String)> {
