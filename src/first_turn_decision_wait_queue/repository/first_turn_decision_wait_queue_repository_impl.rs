...
impl FirstTurnDecisionWaitQueueRepository for FirstTurnDecisionWaitQueueRepositoryImpl {
    async fn enqueue_player_id_for_wait(&self, player_unique_id: i32) -> Result<bool, Box<dyn Error>> {
        ...
        let player_id_waiting_queue = Arc::new(FirstTurnDecisionWaitQueue::new());

        let player_id_waiting_queue_clone = Arc::clone(&player_id_waiting_queue);
        tokio::spawn(async move {
            player_id_waiting_queue_clone.enqueue_player(player_unique_id).await;
            println!("현재 큐에 있는 플레이어 아이디 ->{:?}",player_unique_id);
        }).await.unwrap();
    ...
    async fn enqueue_player_choice_for_wait(&self, player_choice: String) -> Result<bool, Box<dyn Error>> {
        ...
        let choice_waiting_queue = Arc::new(FirstTurnDecisionWaitQueue::new());
        let choice_waiting_queue_clone = Arc::clone(&choice_waiting_queue);
        tokio::spawn(async move {
            choice_waiting_queue_clone.enqueue_player_choice(player_choice).await;
