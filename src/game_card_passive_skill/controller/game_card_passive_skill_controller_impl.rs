...
impl GameCardPassiveSkillController for GameCardActiveSkillControllerImpl {
    async fn request_targeting_active_skill(
        &self, targeting_passive_skill_request_form: TargetingPassiveSkillRequestForm) -> TargetingPassiveSkillResponseForm {

        println!("GameCardActiveSkillControllerImpl: request_targeting_active_skill()");

        // 세션 아이디를 검증합니다.
        let account_unique_id =
            self.is_valid_session(targeting_passive_skill_request_form.to_session_validation_request()).await;

        if account_unique_id == -1 {
            return TargetingPassiveSkillResponseForm::new(false)
        }
        ...
        let is_this_your_turn_response =
            game_protocol_validation_service_guard.is_this_your_turn(
                targeting_passive_skill_request_form
        ...
        let unit_card_index_string = targeting_passive_skill_request_form.get_unit_card_index();
        let unit_card_index = unit_card_index_string.parse::<i32>().unwrap();

        // TODO: 프로토콜 검증 할 때가 아니라 패스

        // Active Skill Summary 획득
        let usage_skill_index_string = targeting_passive_skill_request_form.get_usage_skill_index();
        ...
        let summary_passive_skill_effect_response =
            game_card_passive_skill_service_guard.summary_passive_skill(
                targeting_passive_skill_request_form
                    .to_summary_passive_skill_effect_request(
        ...
        //     *summary_passive_skill_effect_response.get_required_energy().get_required_energy_race();
        // let required_energy_count_to_use_skill =
        //     summary_passive_skill_effect_response.get_required_energy().get_required_energy_count();
        ...
        let target_card_index_string = targeting_passive_skill_request_form.get_opponent_target_card_index();
        let target_card_index = target_card_index_string.parse::<i32>().unwrap();

        // let target_skill_type = summary_passive_skill_effect_response.get_skill_type();
        // let target_skill_damage = summary_passive_skill_effect_response.get_skill_damage();
        ...
        TargetingPassiveSkillResponseForm::new(true)
    }

    async fn request_non_targeting_active_skill(
        &self, non_targeting_passive_skill_request_form: NonTargetingPassiveSkillRequestForm) -> NonTargetingPassiveSkillResponseForm {
        ...
        if account_unique_id == -1 {
            return NonTargetingPassiveSkillResponseForm::new(false)
        }
        ...
        NonTargetingPassiveSkillResponseForm::new(true)
    }
}