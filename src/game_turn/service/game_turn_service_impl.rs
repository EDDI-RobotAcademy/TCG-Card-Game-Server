...
impl GameTurnService for GameTurnServiceImpl {
    async fn decide_first_turn_object(&mut self, decide_first_turn_request: DecideFirstTurnRequest)-> DecideFirstTurnResponse {
        ...
        let session_id1 = decide_first_turn_request.get_session_id1();
        let choice1 = decide_first_turn_request.get_choice1();
        let session_id2 = decide_first_turn_request.get_session_id2();
        let choice2 = decide_first_turn_request.get_choice2();

        let mut game_turn_repository_guard = self.game_turn_repository.lock().await;
        let result = game_turn_repository_guard.decide_first_turn(session_id1,choice1.to_string(),session_id2,choice2.to_string());
