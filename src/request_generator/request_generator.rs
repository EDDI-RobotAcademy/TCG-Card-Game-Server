...
use crate::request_generator::attach_special_energy_request_form_generator::create_attach_special_energy_card_request_form;
...
pub async fn create_request_and_call_service(data: &JsonValue) -> Option<ResponseType> {
    ...
    if let Some(protocol_number) = data.get("protocolNumber").and_then(|v| v.as_i64()) {
        ...
        match protocol_number {
            ...
            1015 => {
                // Opponent Field Unit Energy Removal Item Card Usage
                if let Some(request_form) = create_opponent_field_unit_energy_removal_item_request_form(&data) {
                    ...
                    let response_type = Some(ResponseType::REMOVE_OPPONENT_FIELD_UNIT_ENERGY_ITME_USAGE(response_form));
